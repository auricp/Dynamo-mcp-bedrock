...
import { DynamoDBClient, CreateTableCommand, ListTablesCommand, DescribeTableCommand, UpdateTableCommand, PutItemCommand, GetItemCommand, UpdateItemCommand, QueryCommand, ScanCommand, } from "@aws-sdk/client-dynamodb";
...
async function queryTable(params) {
    try {
        // Ensure expressionAttributeValues is a plain object
        let exprAttrVals = params.expressionAttributeValues;
        if (typeof exprAttrVals === "string") {
            try {
                exprAttrVals = JSON.parse(exprAttrVals);
            }
            catch (err) {
                console.error("Error parsing expressionAttributeValues in queryTable:", err, exprAttrVals);
                exprAttrVals = undefined;
            }
        }
        Object.keys(exprAttrVals || {}).forEach(key => {
            ...
            if (val && typeof val === "object" && (val.N || val.S || val.B)) {
                if (val.N !== undefined)
                    exprAttrVals[key] = Number(val.N);
                else if (val.S !== undefined)
                    exprAttrVals[key] = val.S;
                else if (val.B !== undefined)
                    exprAttrVals[key] = val.B;
            }
        });
        const command = new QueryCommand({
            TableName: params.tableName,
            KeyConditionExpression: params.keyConditionExpression,
            ExpressionAttributeValues: exprAttrVals ? marshall(exprAttrVals) : undefined,
            ExpressionAttributeNames: params.expressionAttributeNames,
        ...
        return {
            ...
            content: [{ type: "text", text: JSON.stringify(response.Items ? response.Items.map(item => unmarshall(item)) : []) }]
        };
    }
    catch (error) {
    ...
    try {
        ...
        if (typeof exprAttrVals === "string") {
            try {
                ...
                console.error("Error parsing expressionAttributeValues in scanTable:", err, exprAttrVals);
                exprAttrVals = undefined;
            }
        }
        Object.keys(exprAttrVals || {}).forEach(key => {
        ...
        if (params.expressionAttributeNames) {
            Object.keys(params.expressionAttributeNames).forEach(key => {
                if (key === "#") {
                    throw new Error('Invalid ExpressionAttributeNames key: "#" is not allowed. Use e.g. "#Amount".');
                }
            });
        }
